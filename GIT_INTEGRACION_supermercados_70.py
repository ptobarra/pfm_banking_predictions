...
def supermercados_70(transacciones_df, balance_df):
    ...
    transacciones_super_df['FECHA'] = pandas.to_datetime(transacciones_super_df['FECHA'], yearfirst=True)
    ...
    valid_prediction_tmp = False

    if final_mean != 0:
        valid_prediction_tmp = True
    ...
    final_year_tmp = final_date_month_df['ds'].dt.year.max()

    # calculo el mes de la predicción
    final_month_tmp = final_date_month_df['ds'].dt.month.max()

    # paso de numero a nombre el mes de la predicción
    datetime_object = datetime.strptime(str(final_month_tmp), "%m")
    final_month_str_tmp = datetime_object.strftime("%B")

    # damos resultados finales
    #     print('Tus gasto aproximado en supermercados en ' + final_month_str_tmp + '-' + str(final_year_tmp) +
    #           ' sera: ' + str(5 * round(final_prediction / 5)) + ' eur')
    mensaje1_str = 'Tus gasto aproximado en supermercados en ' + final_month_str_tmp + '-' + str(
        final_year_tmp) + ' sera: ' + str(5 * round(final_prediction / 5)) + ' eur'
    #     print('Predicción válida: ' + str(valid_prediction_tmp))
    mensaje2_str = 'Predicción válida: ' + str(valid_prediction_tmp)

    return \
        mensaje1_str, \
        mensaje2_str, valid_prediction_tmp, final_prediction, final_year_tmp, final_month_tmp, final_month_str_tmp
...
mensaje1, mensaje2, valid_prediction, final_amount, final_year, final_month, final_month_str = supermercados_70(
